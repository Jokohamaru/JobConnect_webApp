...
export class RecruiterController {
  ...
  findOne(@Param('id') id: string) {
    return this.recruiterService.findOne(id); // Bỏ dấu +
  }

  @Patch(':id')
  update(@Param('id') id: string, @Body() updateRecruiterDto: UpdateRecruiterDto) {
    return this.recruiterService.update(id, updateRecruiterDto); // Bỏ dấu +
  }

  @Delete(':id')
  remove(@Param('id') id: string) {
    return this.recruiterService.remove(id); // Bỏ dấu +
  }
}
